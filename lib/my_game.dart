...
class MyGame extends FlameGame with KeyboardEvents {
    ...
    if (vidas > 0) {
      ...
      if (TiempoFigura > TiempoGenerarFigura) {
        ...
        switch (RanFig.nextInt(16)) {
          case 0:
            add(Pinguino(
                position: Vector2(RanPos.nextDouble() * (size.x - 50), 0),
      ...
      final Flower flower = children.query<Flower>().first;
      var figuras = children.query<PositionComponent>();
      figuras.remove(flower);
      //(element) => (element is PositionComponent && element is! Flower));
      moverFigura(figuras);
    } else if (play) {
  ...
  void moverFigura(List<PositionComponent> figuras) {
    if (figuras.isNotEmpty) {
      for (int i = 0; i < figuras.length; i++) {
        figuras[i].y += vel;

        if (colFig(player, figuras[i])) {
          Puntos++;
          actualizarText();
          remove(figuras[i]);
          aumentarDificultad();
        } else {
          if (figuras[i].y > size.y) {
            remove(figuras[i]);
            vidas--;
          }
        }
